...
const buildEducation = (educations: Education[]) => educations?.map((education, index, arr) => ([
  ...
  {
    text: education.gpa ? `GPA: ${education.gpa}` : null,
    style: 'content',
  },
  index < arr.length - 1 ? {
    text: ' ',
...
export const generatePdf = (body: ResumeBody) => new Promise<any>((resolve) => { // eslint-disable-line @typescript-eslint/no-explicit-any
  generate(body).getBlob(resolve);
});
